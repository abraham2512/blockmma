...
struct blockmma_task{
    int id;
    struct blockmma_data *data;
...
long blockmma_send_task(struct blockmma_cmd __user *user_cmd) {
    ...
    printk("CORE.C: SEND TASK STARTED");
    struct blockmma_task *new_task;
    new_task = kmalloc(sizeof(struct blockmma_task),GFP_KERNEL);
    new_task->id = 0;
    new_task-> data = kmalloc(sizeof(struct blockmma_data),GFP_KERNEL);

    new_task->data->a = kmalloc(sizeof(float)*128*128,GFP_KERNEL);
    new_task->data->b = kmalloc(sizeof(float)*128*128,GFP_KERNEL);
    new_task->data->c = kmalloc(sizeof(float)*128*128,GFP_KERNEL);
    new_task->data->m = user_cmd->m;
    new_task->data->n = user_cmd->n;
    new_task->data->k = user_cmd->k;
    
    printk("CORE.C: STARTED COPY");
    // cmd->a = (float*)malloc(128*128*sizeof(float))

    if(copy_from_user(new_task->data->a, (void *)user_cmd->a, sizeof(float)*128*128)!=0){
        printk("Could not copy");
        return -EFAULT;
    };
    if(copy_from_user(new_task->data->b, (void *)user_cmd->b, sizeof(float)*128*128)!=0){
        printk("Could not copy");
        return -EFAULT;
    };
    if(copy_from_user(new_task->data->c, (void *)user_cmd->c, sizeof(float)*128*128)!=0){
        printk("Could not copy");
        return -EFAULT;
    };
